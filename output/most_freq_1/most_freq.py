...
def run(tokens):
    ...
    classifier_weights = pd.read_csv(
        "output\most_freq_1\most_freq_weights.csv",
        index_col=[0, 1],
        dtype={"feature": str},
    )
    # inputs #####################################################
    ...
    def mlp_0_0(attn_0_0_output, position):
        ...
        if key in {
            ...
            return 6
        elif key in {("4", 4), ("4", 6), ("5", 2), ("5", 3), ("5", 4)}:
            return 5
        elif key in {("5", 5)}:
            return 2
        elif key in {("5", 0)}:
    ...
    def mlp_0_1(attn_0_4_output, position):
        ...
        if key in {
            ...
            return 3
        elif key in {("0", 3), ("1", 3), ("2", 3), ("4", 3), ("5", 3), ("<s>", 3)}:
            return 4
        return 2
    ...
    def mlp_0_3(attn_0_5_output, position):
        ...
        if key in {
            ...
            return 7
        elif key in {("0", 2), ("1", 2), ("2", 2), ("5", 2), ("<s>", 2)}:
            return 1
        elif key in {("3", 2)}:
            return 6
        return 4
    ...
    # num_mlp_0_0 #################################################
    def num_mlp_0_0(num_attn_0_6_output):
        key = num_attn_0_6_output
        if key in {0}:
            return 2
        return 0

    num_mlp_0_0_outputs = [num_mlp_0_0(k0) for k0 in num_attn_0_6_outputs]
    num_mlp_0_0_output_scores = classifier_weights.loc[
        [("num_mlp_0_0_outputs", str(v)) for v in num_mlp_0_0_outputs]
    ]

    # num_mlp_0_1 #################################################
    def num_mlp_0_1(num_attn_0_6_output):
        key = num_attn_0_6_output
        if key in {0}:
            return 3
        return 6

    num_mlp_0_1_outputs = [num_mlp_0_1(k0) for k0 in num_attn_0_6_outputs]
    ...
    # num_mlp_0_3 #################################################
    def num_mlp_0_3(num_attn_0_6_output, num_attn_0_1_output):
    ...
    # attn_1_1 ####################################################
    def predicate_1_1(mlp_0_1_output, attn_0_4_output):
        if mlp_0_1_output in {0, 2}:
    ...
    attn_1_1_pattern = select_closest(attn_0_4_outputs, mlp_0_1_outputs, predicate_1_1)
    ...
    def predicate_1_3(mlp_0_2_output, token):
        if mlp_0_2_output in {0, 2, 3, 4, 5, 6}:
            return token == "2"
    ...
    # attn_1_5 ####################################################
    def predicate_1_5(mlp_0_2_output, position):
        if mlp_0_2_output in {0, 1, 5, 6}:
            return position == 6
        elif mlp_0_2_output in {2}:
            return position == 1
        elif mlp_0_2_output in {3, 4}:
            return position == 7
        elif mlp_0_2_output in {7}:
            return position == 5

    attn_1_5_pattern = select_closest(positions, mlp_0_2_outputs, predicate_1_5)
    ...
    # attn_1_6 ####################################################
    def predicate_1_6(token, mlp_0_2_output):
        if token in {"0"}:
    ...
    attn_1_6_pattern = select_closest(mlp_0_2_outputs, tokens, predicate_1_6)
    ...
    # attn_1_7 ####################################################
    def predicate_1_7(mlp_0_2_output, position):
        if mlp_0_2_output in {0, 4, 6, 7}:
            return position == 5
    ...
    attn_1_7_pattern = select_closest(positions, mlp_0_2_outputs, predicate_1_7)
    ...
    # num_attn_1_1 ####################################################
    def num_predicate_1_1(attn_0_5_output, token):
    ...
    num_attn_1_1_pattern = select(tokens, attn_0_5_outputs, num_predicate_1_1)
    ...
    def num_predicate_1_2(attn_0_3_output, token):
        if attn_0_3_output in {"0", "5", "4", "2", "1", "<s>"}:
            return token == "3"
    ...
    # num_attn_1_5 ####################################################
    def num_predicate_1_5(attn_0_3_output, token):
        if attn_0_3_output in {"0"}:
            return token == "3"
        elif attn_0_3_output in {"5", "4", "2", "1", "3"}:
            return token == "0"
        elif attn_0_3_output in {"<s>"}:
            return token == "<s>"

    num_attn_1_5_pattern = select(tokens, attn_0_3_outputs, num_predicate_1_5)
    ...
    # mlp_1_0 #####################################################
    def mlp_1_0(attn_1_7_output, attn_1_1_output):
        key = (attn_1_7_output, attn_1_1_output)
        return 2

    mlp_1_0_outputs = [
        mlp_1_0(k0, k1) for k0, k1 in zip(attn_1_7_outputs, attn_1_1_outputs)
    ]
    ...
    # mlp_1_1 #####################################################
    def mlp_1_1(attn_1_7_output, attn_0_2_output):
        ...
        if key in {
            ...
            return 1
        elif key in {("0", "3"), ("0", "4"), ("4", "0")}:
            return 0
        return 7

    mlp_1_1_outputs = [
        mlp_1_1(k0, k1) for k0, k1 in zip(attn_1_7_outputs, attn_0_2_outputs)
    ]
    ...
    # mlp_1_2 #####################################################
    def mlp_1_2(attn_1_7_output, attn_0_5_output):
    ...
    mlp_1_2_outputs = [
        mlp_1_2(k0, k1) for k0, k1 in zip(attn_1_7_outputs, attn_0_5_outputs)
    ]
    ...
    # mlp_1_3 #####################################################
    def mlp_1_3(position, attn_1_7_output):
        ...
        if key in {
            ...
            return 6
        elif key in {(0, "0"), (0, "1"), (0, "3"), (0, "4"), (0, "5"), (0, "<s>")}:
            return 2
        elif key in {
            (0, "2"),
            (2, "2"),
    ...
    mlp_1_3_outputs = [mlp_1_3(k0, k1) for k0, k1 in zip(positions, attn_1_7_outputs)]
    ...
    # num_mlp_1_1 #################################################
    def num_mlp_1_1(num_attn_0_7_output):
    ...
    num_mlp_1_1_outputs = [num_mlp_1_1(k0) for k0 in num_attn_0_7_outputs]
    ...
    # attn_2_0 ####################################################
    def predicate_2_0(position, mlp_0_3_output):
        if position in {0, 1, 5, 6}:
    ...
    attn_2_0_pattern = select_closest(mlp_0_3_outputs, positions, predicate_2_0)
    attn_2_0_outputs = aggregate(attn_2_0_pattern, mlp_1_3_outputs)
    ...
    def predicate_2_1(mlp_0_2_output, token):
        if mlp_0_2_output in {0}:
            return token == "5"
        elif mlp_0_2_output in {1, 4, 5, 6, 7}:
            return token == "2"
        elif mlp_0_2_output in {2}:
            return token == "0"
        elif mlp_0_2_output in {3}:
    ...
    def predicate_2_2(mlp_0_2_output, token):
        if mlp_0_2_output in {0, 2, 4}:
            return token == "2"
        elif mlp_0_2_output in {1, 5}:
            return token == "0"
        elif mlp_0_2_output in {3, 6}:
            return token == "1"
        elif mlp_0_2_output in {7}:
            return token == "5"
    ...
    def predicate_2_3(attn_0_1_output, mlp_0_0_output):
        if attn_0_1_output in {"0", "5", "2", "1", "3"}:
    ...
    attn_2_4_outputs = aggregate(attn_2_4_pattern, num_mlp_0_0_outputs)
    attn_2_4_output_scores = classifier_weights.loc[
        [("attn_2_4_outputs", str(v)) for v in attn_2_4_outputs]
    ]

    # attn_2_5 ####################################################
    def predicate_2_5(q_mlp_0_2_output, k_mlp_0_2_output):
        if q_mlp_0_2_output in {0, 1, 3, 4, 5, 6, 7}:
            return k_mlp_0_2_output == 7
        elif q_mlp_0_2_output in {2}:
            return k_mlp_0_2_output == 4

    attn_2_5_pattern = select_closest(mlp_0_2_outputs, mlp_0_2_outputs, predicate_2_5)
    ...
    # attn_2_6 ####################################################
    def predicate_2_6(position, mlp_0_3_output):
    ...
    attn_2_6_pattern = select_closest(mlp_0_3_outputs, positions, predicate_2_6)
    ...
    # attn_2_7 ####################################################
    def predicate_2_7(attn_1_4_output, attn_0_3_output):
        if attn_1_4_output in {0, 1, 2, 3, 7}:
            return attn_0_3_output == "0"
        elif attn_1_4_output in {4, 5, 6}:
            return attn_0_3_output == "<s>"

    attn_2_7_pattern = select_closest(attn_0_3_outputs, attn_1_4_outputs, predicate_2_7)
    attn_2_7_outputs = aggregate(attn_2_7_pattern, mlp_1_2_outputs)
    ...
    def num_predicate_2_0(attn_0_1_output, token):
        if attn_0_1_output in {"0"}:
            return token == "<s>"
        elif attn_0_1_output in {"1", "3", "4", "<s>"}:
            return token == "2"
    ...
    # num_attn_2_3 ####################################################
    def num_predicate_2_3(mlp_0_3_output, token):
        if mlp_0_3_output in {0, 4, 5}:
            return token == "<s>"
        elif mlp_0_3_output in {1, 2, 6, 7}:
            return token == "4"
        elif mlp_0_3_output in {3}:
            return token == "3"

    num_attn_2_3_pattern = select(tokens, mlp_0_3_outputs, num_predicate_2_3)
    ...
    def num_predicate_2_5(attn_0_1_output, token):
        if attn_0_1_output in {"0"}:
            return token == "0"
        elif attn_0_1_output in {"2", "1", "4", "5"}:
            return token == "3"
        elif attn_0_1_output in {"3"}:
            return token == "4"
        elif attn_0_1_output in {"<s>"}:
    ...
    # mlp_2_0 #####################################################
    def mlp_2_0(attn_1_2_output, attn_0_5_output):
        key = (attn_1_2_output, attn_0_5_output)
        if key in {
            ("0", "3"),
            ("1", "3"),
            ("1", "4"),
    ...
    mlp_2_0_outputs = [
        mlp_2_0(k0, k1) for k0, k1 in zip(attn_1_2_outputs, attn_0_5_outputs)
    ]
    ...
    # mlp_2_1 #####################################################
    def mlp_2_1(attn_0_4_output, token):
    ...
    mlp_2_1_outputs = [mlp_2_1(k0, k1) for k0, k1 in zip(attn_0_4_outputs, tokens)]
    mlp_2_1_output_scores = classifier_weights.loc[
        [("mlp_2_1_outputs", str(v)) for v in mlp_2_1_outputs]
    ]

    # mlp_2_2 #####################################################
    def mlp_2_2(attn_1_7_output, attn_0_5_output):
        key = (attn_1_7_output, attn_0_5_output)
        if key in {
            ("0", "0"),
            ("0", "1"),
    ...
    mlp_2_2_outputs = [
        mlp_2_2(k0, k1) for k0, k1 in zip(attn_1_7_outputs, attn_0_5_outputs)
    ]
    ...
    # mlp_2_3 #####################################################
    def mlp_2_3(attn_0_2_output, attn_2_1_output):
        key = (attn_0_2_output, attn_2_1_output)
        return 0

    mlp_2_3_outputs = [
        mlp_2_3(k0, k1) for k0, k1 in zip(attn_0_2_outputs, attn_2_1_outputs)
    ]
    ...
    # num_mlp_2_0 #################################################
    def num_mlp_2_0(num_attn_1_5_output, num_attn_0_0_output):
        key = (num_attn_1_5_output, num_attn_0_0_output)
        return 2

    num_mlp_2_0_outputs = [
        num_mlp_2_0(k0, k1)
        for k0, k1 in zip(num_attn_1_5_outputs, num_attn_0_0_outputs)
    ]
    ...
    # num_mlp_2_3 #################################################
    def num_mlp_2_3(num_attn_1_2_output):
        key = num_attn_1_2_output
        if key in {0}:
            return 7
        return 1

    num_mlp_2_3_outputs = [num_mlp_2_3(k0) for k0 in num_attn_1_2_outputs]
